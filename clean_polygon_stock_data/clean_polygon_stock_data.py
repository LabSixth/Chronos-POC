...
def main() -> None:
    """
    Glue job entrypoint:
      - Read quarterly JSONs from s3://polygon-data-stock/full/
      - Enrich + nest into json struct
      - Flatten into individual columns (no company column)
      - Write combined_data.parquet under s3://polygon-data-stock/output/
      - Remove the leftover temp files under output/_temp_write/
    """
    glue_context, job = initialize_glue_job()

    input_paths = [
        "s3://polygon-data-stock/full/AAPL_Q1_2024.json",
        "s3://polygon-data-stock/full/AAPL_Q2_2024.json",
        "s3://polygon-data-stock/full/AAPL_Q3_2024.json",
        "s3://polygon-data-stock/full/AAPL_Q4_2024.json",
    ]
    enriched = load_and_enrich_json(glue_context, input_paths)
    flat = flatten_json_struct(enriched)

    temp_output = "s3://polygon-data-stock/output/_temp_write"
    final_output = "s3://polygon-data-stock/output/combined_data.parquet"
